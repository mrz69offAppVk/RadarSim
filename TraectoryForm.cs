...
namespace Radar
{
    public partial class TraectoryForm : Form
    {
        public TraectoryForm()
        {
            InitializeComponent();
            DrawingGrid();
        }
        void DrawingGrid() {
            ...
            pictureBoxTrDr.BackgroundImage = radar.Grid(pictureBoxTrDr.Width, pictureBoxTrDr.Height, 15);
        }
        List<Point> list = new List<Point>();
        private bool flag = false;
        private bool isNewObjStarted = false;
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            int count = 0;
            PointF point = new PointF(pictureBoxTrDr.Width / 2, pictureBoxTrDr.Height / 2);
            PolarCoordinate polar = new PolarCoordinate();
            float km = (((float)pictureBoxTrDr.Height) / (15 * 10)) / 2;
            foreach (Point h in list)
            {
                count++;
                double[] ar = polar.GetPolar(point, h.X, h.Y);
            }
        ...
        {
            Airplane.Namber = Convert.ToInt32(textBox1.Text);pictureBoxTrDr.Refresh();
        }

        private void TextBox2_TextChanged(object sender, EventArgs e)
        {
            Airplane.StartAzimuth = Convert.ToDouble(textBox2.Text); pictureBoxTrDr.Refresh();
        }

        private void TextBox3_TextChanged(object sender, EventArgs e)
        {
            Airplane.StartDistance = Convert.ToDouble(textBox3.Text); pictureBoxTrDr.Refresh();
        }

        private void TextBox4_TextChanged(object sender, EventArgs e)
        {
            Airplane.EndAzimuth = Convert.ToDouble(textBox4.Text); pictureBoxTrDr.Refresh();
        }

        private void TextBox5_TextChanged(object sender, EventArgs e)
        {
            Airplane.EndDistance = Convert.ToDouble(textBox5.Text); pictureBoxTrDr.Refresh();
        }

        private void TextBox6_TextChanged(object sender, EventArgs e)
        {
            Airplane.Speed = 0.0001f * Convert.ToSingle(textBox6.Text); pictureBoxTrDr.Refresh();
        }
        ...
        private void PictureBoxTrDr_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            PointF point = new PointF(pictureBoxTrDr.Width / 2, pictureBoxTrDr.Height / 2);
            PolarCoordinate polar = new PolarCoordinate();
            double[] ar = polar.GetPolar(point, ex, ey);
        ...
        private void PictureBoxTrDr_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                if (flag)
                {
                    Bitmap bmp = new Bitmap(pictureBoxTrDr.Width, pictureBoxTrDr.Height);
                    Graphics g = Graphics.FromImage(bmp);
                    list.Add(new Point(e.X, e.Y));
                    g.DrawLines(new Pen(Color.FromArgb(255, 170, 130, 0), 3), list.ToArray());
                    image = pictureBoxTrDr.Image = bmp;
                }
                else
                {
                    list.Add(new Point(e.X, e.Y));
            ...
            ex = e.X; ey = e.Y;
            pictureBoxTrDr.Refresh();
        }

        private void PictureBoxTrDr_MouseUp(object sender, MouseEventArgs e)
        {
            float km = (((float)pictureBoxTrDr.Height) / (15 * 10)) / 2;
            PointF point = new PointF(pictureBoxTrDr.Width / 2, pictureBoxTrDr.Height / 2);
            PolarCoordinate polar = new PolarCoordinate();
            ...
            {
                count++;
                double[] array = polar.GetPolar(point, h.X, h.Y);
            }
            Airplane.Lenght = count;
            Airplane.Trajectory = new double[count, 2];
        ...
        private void PictureBoxTrDr_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Airplane.EndAzimuth != o && Airplane.EndDistance != o) { radar.Point(e.Graphics, pictureBoxTrDr.Width, pictureBoxTrDr.Height, 15, Airplane.EndAzimuth, Airplane.EndDistance, "Конец координат"); }
        }
        ...
        private void pictureBoxTrDr_MouseClick(object sender, MouseEventArgs e)
        {
            if (!isNewObjStarted)
            {
                AddTargetPoint(e.X, e.Y);
                isNewObjStarted = true;
            }
        }

        private void AddTargetPoint(double x, double y)
        {
            RadarDraw.DrawPoint(x, y);
            PointF PFCenter = default;
            TrainingObject trainingObject = new TrainingObject(0,0, PFCenter, x, y);
        }
    }
}
